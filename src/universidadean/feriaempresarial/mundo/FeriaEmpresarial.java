...
public class FeriaEmpresarial {
	 ...
	 * El arreglo de puestos de la feria fue inicializado según el total de
	 * puestos<br>
	 */
	public FeriaEmpresarial() {
		empresas = new ArrayList<>();
		puestos = new Puesto[NUM_PUESTOS_NORTE + NUM_PUESTOS_SUR + NUM_PUESTOS_ORIENTE + NUM_PUESTOS_OCCIDENTE
				+ NUM_PUESTOS_CENTRO];
		crearPuestos();
	}
	 ...
	 * <b>post: </b>Se agregó una empresa asistente a la feria con los valores dados
	 * por parámetro<br>
	 * El tipo de la empresa se ha inicializado como visitante<br>
	 * 
	 * @param nNombreEmpresa            Es el nombre de la empresa. nNombreEmpresa
	 *                                  != null,nNombreEmpresa != ""
	 * @param nNumeroPersonasAsistentes Es el número de personas de la empresa
	 *                                  asistentes a la feria.
	 *                                  nNumeroPersonasAsistentes > 0
	 * @throws Exception <br>
	 *                   1. Si ya existe una empresa con el nombre ingresado<br>
	 *                   2. Si el numero de personas asistentes excede el numero
	 *                   máximo de personas asistentes
	 */
	 ...
	 * <b>pre: </b>La lista de empresas y los puestos han sido inicializados<br>
	 * <b>post: </b>Se ha adicionado una empresa expositora a la contenedora de
	 * empresas con los valores dados por parámetro<br>
	 * El tipo de la empresa se ha inicializado como expositora<br>
	 * Se ha ocupado el puesto indicado por parámetro<br>
	 * 
	 * @param nNombreEmpresa             Es el nombre de la empresa. nNombreEmpresa
	 *                                   != null,nNombreEmpresa != ""
	 * @param nNumeroPersonasExpositoras Es el número de personas de la empresa
	 *                                   expositora . nNumeroPersonasExpositoras > 0
	 * @param zonaPuesto                 Es la zona donde se ubicará la empresa.
	 *                                   zonaPuesto != null, zonaPuesto != ""
	 * @param numeroPuesto               Es el número del puesto sobre la zona.
	 *                                   numeroPuesto > 0
	 * @throws Exception <br>
	 *                   1. Si ya existe una empresa con el nombre ingresado <br>
	 *                   3. No existe el puesto escogido <br>
	 *                   4. Si el puesto escogido ya esta ocupado <br>
	 *                   5. Si el numero de personas expositoras no esta dentro del
	 *                   rango de personas permitidas en el puesto
	 */
	public void ingresarEmpresaExpositora(String nNombreEmpresa, int nNumeroPersonasExpositoras, String zonaPuesto,
			int numeroPuesto) throws Exception {
		boolean encontrado = false;
	 ...
	 * La empresa cuyo nombre ha sido dado por parámetro ha sido eliminada de la
	 ...
	 * El puesto sugerido es aquel que primero puede albergar la cantidad de
	 * expositores y está disponible<br>
	 * El recorrido por zonas se hace en el siguiente orden: Norte, Oriente, Sur,
	 * Occidente, Centro.
	 * 
	 * @param numeroExpositores > 0
	 * @return Puesto sugerido para la cantidad de expositores. null si no hay
	 *         puesto disponible que albergue a los expositores.
	 */
	 ...
	 * <b>pre: </b> El arreglo de puestos tiene tamaño NUM_PUESTOS_NORTE +
	 * NUM_PUESTOS_SUR + NUM_PUESTOS_ORIENTE + NUM_PUESTOS_OCCIDENTE +
	 * NUM_PUESTOS_CENTRO<br>
	 * <b>post: </b> Los puestos han sido inicializados con sus valores de acuerdo a
	 * su zona de ubicación<br>
	 * Los puestos se llenan en el siguiente orden por zonas: Norte, Oriente, Sur,
	 * Occidente, Centro. Cada zona se llena por número de puesto incrementálmente
	 */
	private void crearPuestos() {
		...
		for (int i = 0; i < NUM_PUESTOS_SUR; i++) {
			puestos[i + NUM_PUESTOS_NORTE + NUM_PUESTOS_ORIENTE] = new Puesto(numPuesto, zona, MIN_PERSONAS_SUR,
					MAX_PERSONAS_SUR);
			numPuesto++;
		}
		...
		for (int i = 0; i < NUM_PUESTOS_OCCIDENTE; i++) {
			puestos[i + NUM_PUESTOS_NORTE + NUM_PUESTOS_ORIENTE + NUM_PUESTOS_SUR] = new Puesto(numPuesto, zona,
		...
		for (int i = 0; i < NUM_PUESTOS_CENTRO; i++) {
			puestos[i + NUM_PUESTOS_NORTE + NUM_PUESTOS_ORIENTE + NUM_PUESTOS_SUR + NUM_PUESTOS_OCCIDENTE] = new Puesto(
					numPuesto, zona, MIN_PERSONAS_CENTRO, MAX_PERSONAS_CENTRO);
			numPuesto++;
		}
	}
     ...
     * Metodo que devuelve la zona con mayor número de expositores
     * @return nombre de la zona
     */
    public String metodo1() {
    	
        int zonaNorte = 0;	 //"Zona norte";
        int zonaSur = 0;	// "Zona sur";
        int zonaOriente = 0; // "Zona oriente";
        int zonaOccidente = 0;	// "Zona occidente";
        int zonaCentro = 0; // "Zona centro";

    	for(Empresa e : empresas) {
    		if(e.darTipo().equals(Empresa.EXPOSITORA)) {
    			for (Puesto puesto : puestos) {
    	            if (puesto.estaOcupado()) {
    	            	if(puesto.darNombreEmpresa().equals(e.darNombre())){
    	            		switch(puesto.darZona()) {
    	            		case "Zona norte":
    	            			zonaNorte+=e.darNumeroPersonasAsistentes();
    	            			break;
    	            		case "Zona sur":
    	            			zonaSur+=e.darNumeroPersonasAsistentes();
    	            			break;
    	            		case "Zona oriente":
    	            			zonaOriente+=e.darNumeroPersonasAsistentes();
    	            			break;
    	            		case "Zona occidente":
    	            			zonaOccidente+=e.darNumeroPersonasAsistentes();
    	            			break;
    	            		case "Zona centro":
    	            			zonaCentro+=e.darNumeroPersonasAsistentes();
    	            			break;
    	            		default:
    	            			// no suma
    	...
    	if(zonaCentro==0 && zonaCentro==0 && zonaCentro==0 && zonaCentro==0)
    		return "NO SE HAN CARGADO EMPRESAS EXPOSITORAS";
    	else if(zonaNorte>=zonaSur && zonaNorte>=zonaOriente && zonaNorte>=zonaOccidente && zonaNorte>=zonaCentro)
    		return "ZONA NORTE"+zonaNorte;
    	else if(zonaSur>=zonaNorte && zonaSur>=zonaOriente && zonaSur>=zonaOccidente && zonaSur>=zonaCentro)
    		return "ZONA SUR"+zonaSur;
    	else if(zonaOriente>=zonaNorte && zonaOriente>=zonaSur && zonaOriente>=zonaOccidente && zonaOriente>=zonaCentro)
    		return "ZONA ORIENTE"+zonaOriente;
    	else if(zonaOccidente>=zonaNorte && zonaOccidente>=zonaSur && zonaOccidente>=zonaOriente && zonaOccidente>=zonaCentro)
    		return "ZONA OCCIDENTE"+zonaOccidente;
    	else if(zonaCentro>=zonaNorte && zonaCentro>=zonaSur && zonaCentro>=zonaOccidente && zonaCentro>=zonaOriente)
    		return "ZONA CENTRO"+zonaCentro;    	
    	else
    		return "Revisar";    	
    }
	...
	public String metodo2() {
		...
		for(Empresa e : empresas) {
    		if(e.darTipo().equals(Empresa.EXPOSITORA)) {
    			for (Puesto puesto : puestos) {
    	            if (puesto.estaOcupado()) {
    	            	if(puesto.darNombreEmpresa().equals(e.darNombre())){
    	            		numeroExpositores += e.darNumeroPersonasAsistentes();
    	            	}
    	            }
    			}
    		}
		...
		porcentaje= (float)numeroExpositores/totalExpositores;
		
		if((porcentaje*100) >= 60) {
			return "El punto de equilibrio se ha cumplido "+numeroExpositores+"/"+totalExpositores;
		}else {
			return "El punto de equilibrio no se ha cumplido"+numeroExpositores+"/"+totalExpositores;
		}
